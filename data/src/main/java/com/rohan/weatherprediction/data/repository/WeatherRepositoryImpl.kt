...
class WeatherRepositoryImpl(
    ...
    override suspend fun getCurrentWeather(cityName: String): ResultState<CurrentWeatherEntity> {
        return try {
            val response = remote.getCurrentByCityName(cityName = cityName, appId = BuildConfig.API_KEY)
            if (response != null) {
    ...
    override suspend fun getForecastWeather(cityName: String): ResultState<ForecastEntity> {
        return try {
            val response = remote.getForecastByCityName(cityName = cityName, appId = BuildConfig.API_KEY)
            if (response != null) {
